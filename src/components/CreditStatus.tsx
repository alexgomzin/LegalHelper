...
export default function AnalysisStatus({ className = '', compact = false, onNoCredits }: CreditStatusProps) {
    ...
    return (
      <div className={className}>
        <div className="flex items-center space-x-2 mb-2">
          ...
          <span className="text-yellow-700 text-sm font-medium">Admin: Unlimited analyses</span>
        </div>
        <p className="text-xs text-gray-500 mb-3">You have unlimited document analyses as an admin user.</p>
        <Link
          href="/admin"
    ...
    return (
      <div className={`text-sm ${className}`}>
        {status?.tier === 'admin' ? (
          ...
          <span className="text-green-600 font-medium">Monthly Subscription</span>
        ) : (
          <span className="text-gray-600">
            {status?.analysesRemaining || 0} {(status?.analysesRemaining || 0) === 1 ? 'analysis' : 'analyses'} left
          </span>
        )}
      </div>
    );
  ...
  return (
    <div className={`rounded-lg border border-gray-200 bg-white p-4 ${className}`}>
      <h3 className="font-medium text-gray-800 mb-2">Your Analysis Plan</h3>
      ...
      {status?.tier === 'admin' ? (
        <div>
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="text-purple-700 text-sm font-medium">Admin Account - Unlimited Access</span>
          </div>
          <p className="text-xs text-gray-500 mb-3">You have unlimited document analyses</p>
        </div>
      ) : status?.isSubscribed ? (
        <div>
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="text-green-700 text-sm font-medium">Monthly Subscription Active</span>
          </div>
          <p className="text-xs text-gray-500 mb-3">{status?.analysesRemaining || 0} analyses remaining this month</p>
        </div>
      ) : (
        <div>
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="text-gray-700 text-sm">
              {(status?.analysesRemaining || 0) === 1 ? 'Analysis' : 'Analyses'} remaining
            </span>
          </div>
          <p className="text-xs text-gray-500 mb-3">
            {(status?.analysesRemaining || 0) === 0 ? 'Purchase more analyses to continue using our services.' : ''}
          </p>
        </div>
      )}
      ...
      >
        {status?.tier === 'admin' ? 'Admin Dashboard' : status?.isSubscribed ? 'Manage subscription' : 'Purchase analyses'}
      </Link>
    </div>
  );
} 