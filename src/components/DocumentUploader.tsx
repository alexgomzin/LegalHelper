  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        onError('Upload failed: ' + (error.message || 'Unknown error'))
      }
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
        ...
        <ul className="mt-2 text-sm text-blue-700 list-disc list-inside ml-1 space-y-1">
          <li>Upload a <strong>text-based PDF</strong> document</li>
          <li>Scanned documents may not process correctly</li>
      ...
      <div className="text-sm text-gray-500 mt-4">
        <h4 className="font-medium text-gray-700 mb-2">For best results:</h4>
        <ul className="list-disc list-inside space-y-1 ml-1">
          <li>Use text-based PDFs instead of scanned documents</li>
          <li>Ensure documents are not password protected</li>
          <li>Make sure document text is clear and properly formatted</li>
        </ul>
      </div>
    </div>
  )
