  ...
  try {
    ...
    switch (event_type) {
      case 'transaction.created':
        // Transaction created (draft state) - log but don't give credits yet
        console.log('Transaction created (draft) - waiting for completion:', data.id);
        await logWebhookEvent('transaction.created', { transactionId: data.id, userId: data.custom_data?.user_id, status: data.status });
        break;
