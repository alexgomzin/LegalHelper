...
// Direct function to deduct credit (for internal use)
export async function deductCredit(user_id: string, document_id: string) {
  console.log('Direct deductCredit called for user_id:', user_id, 'document_id:', document_id);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await deductCredit(user_id, document_id);
    
    if (result.success) {
      return res.status(200).json(result);
    } else {
      const statusCode = result.error === 'User profile not found' ? 404 : 
                        result.error === 'No credits remaining' ? 400 : 500;
