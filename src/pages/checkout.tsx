...
export default function Checkout() {
            ...
            window.Paddle.Initialize({
              ...
              eventCallback: function(data: any) {
                ...
                if (data.name === 'checkout.completed') {
                  console.log('🎉 CHECKOUT COMPLETED EVENT - TEMPORARILY DISABLED');
                  console.log('Event data:', JSON.stringify(data, null, 2));
                  console.log('❌ IGNORING ALL CHECKOUT.COMPLETED EVENTS UNTIL FIXED');
                  // TEMPORARILY DISABLE ALL checkout.completed handling
                  // This prevents fake redirects while we debug the real issue
                  return;
                }
                ...
                if (data.name === 'checkout.warning') {
                  console.warn('⚠️ CHECKOUT WARNING EVENT:', data);
                }
                
                // Log all other events for debugging
                if (!['checkout.completed', 'checkout.error', 'checkout.warning'].includes(data.name)) {
                  console.log('ℹ️ OTHER PADDLE EVENT:', data.name, data);
                }
              }
            });
  ...
  const initiatePaddleCheckout = async () => {
    ...
    console.log('checkoutAttempted:', checkoutAttempted);
    
    // Prevent multiple checkout attempts
    if (checkoutAttempted) {
      console.log('❌ Checkout already attempted, preventing duplicate');
      return;
    }
    ...
    try {
      // TEMPORARY FIX: Skip Paddle.js and use API approach directly
      // This bypasses the Paddle.js 400 error by using our API endpoint
      console.log('Bypassing Paddle.js due to 400 error - using API approach directly...');
      
      // Log the data that would have been sent to Paddle.js for debugging
      ...
      console.log('=== DETAILED CHECKOUT DATA (for debugging) ===');
      ...
      // Use hosted checkout API to create proper payment page
      console.log('Creating Paddle hosted checkout...');
      ...
      if (data.success) {
        ...
        const errorDetails = data.details || data.paddleResponse?.error?.detail || '';
        const fullError = `${errorMessage}${errorDetails ? ': ' + errorDetails : ''}`;
        
        console.error('Setting checkout error:', fullError);
        setCheckoutError(fullError);
        
        // Do NOT redirect anywhere - stay on the checkout page and show the error
        console.log('Staying on checkout page to show error');
      }
    } catch (error) {
      ...
      setIsProcessing(false);
      // Reset checkout attempted flag on error so user can try again
      if (checkoutError) {
  ...
  const customerEmail = email || user?.email;
  // Show debug customer details only in development environment
